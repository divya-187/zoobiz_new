				...
				for ($tf = 0; $tf < count($dataArray); $tf++) {
					...
					if ($user_found) {
						...
						if ($feed["user_mobile"] != "" && $feed["mobile_privacy"] == false) {
							$feed["share_timeline_text"] .= "\nMobile No. : " . $feed["user_mobile"];
						}

						if ($feed["sub_category_name"] != "") {
							$feed["share_timeline_text"] .= "\nBusiness Category : " . html_entity_decode($feed["sub_category_name"]);
						}if ($feed["company_name"] != "") {
							$feed["share_timeline_text"] .= "\nCompany Name : " . html_entity_decode($feed["company_name"]);
						}
...
and timeline_master.user_id not in ($blocked_users) ");
			...
			if (count($dataArray) > 0) {
				...
				for ($l = 0; $l < count($dataArray); $l++) {
					...
					$feed["share_timeline_text"] .= "\nName : " . $u_data['user_full_name'];
					if ($u_data["user_mobile"] != "" && $u_data["mobile_privacy"] == 1) {
						$feed["u_data"] .= "\nMobile No. : " . $u_data["user_mobile"];
					}

					if ($u_data["sub_category_name"] != "") {
						$feed["share_timeline_text"] .= "\nBusiness Category : " . html_entity_decode($u_data["sub_category_name"]);
					}if ($u_data["company_name"] != "") {
						$feed["share_timeline_text"] .= "\nCompany Name : " . html_entity_decode($u_data["company_name"]);
					}
...
//23oct2020
					...
					if ($feed["sub_category_name"] != "") {
						$feed["share_timeline_text"] .= "\nCategory Name : " . html_entity_decode($feed["sub_category_name"]);
					}if ($feed["company_name"] != "") {
						$feed["share_timeline_text"] .= "\nCompany Name : " . html_entity_decode($feed["company_name"]);
					}
					$feed["short_name"] = strtoupper(substr($data_notification["user_first_name"], 0, 1) . substr($data_notification["user_last_name"], 0, 1));
