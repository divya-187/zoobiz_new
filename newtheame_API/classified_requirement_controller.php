...
if (isset($_POST) && !empty($_POST)) {
	if ($key == $keydb) {
		...
		if ($_POST['getCllassifiedNew'] == "getCllassifiedNew" && filter_var($user_id, FILTER_VALIDATE_INT) == true) {
			...
			$zoobiz_settings_master = $d->selectRow("classified_city_wise", "zoobiz_settings_master", "", "");
			$zoobiz_settings_masterData = mysqli_fetch_array($zoobiz_settings_master);

			if ($zoobiz_settings_masterData['classified_city_wise'] == 0 && $city_id != null) {
				$appendCityQuery = " and cllassifieds_city_master.city_id = '$city_id' ";
			}
			...
			$qcnt = $d->select(
				...
				"users_master.user_id =cllassifieds_master.user_id AND users_master.active_status=0 and  cllassifieds_master.active_status=0  AND cllassifieds_master.cllassified_id=cllassifieds_city_master.cllassified_id and cllassifieds_master.user_id not in ($blocked_users)   $query2 $appendCityQuery",
    ...
    business_categories.business_category_id=cllassifieds_master.business_category_id AND  cllassifieds_master.cllassified_id='$data[cllassified_id]' "
					...
					while ($feeData122 = mysqli_fetch_array($fi122)) {
						...
						if (!in_array($feeData122['business_category_id'] . '_' . $feeData122['business_sub_category_id'], $catArr)) {
							...
							$category_array["category_name"] = html_entity_decode($feeData122['category_name']);
							$category_array["categoryName"] = html_entity_decode($feeData122['sub_category_name']) . ' - ' . html_entity_decode($feeData122['category_name']);
    ...
    business_categories.business_category_id=classified_category_master.business_category_id AND  classified_category_master.classified_id='$data[cllassified_id]' "
					...
					while ($feeData1 = mysqli_fetch_array($fi1)) {
						...
						if (!in_array($feeData1['business_category_id'] . '_' . $feeData1['business_sub_category_id'], $catArr)) {
							...
							$category_array["category_name"] = html_entity_decode($feeData1['category_name']);

							$category_array["categoryName"] = html_entity_decode($feeData1['category_name']) . ' - ' . html_entity_decode($feeData1['sub_category_name']);
					...
					*/
					...
					$discussion_title = $data["cllassified_title"] = htmlspecialchars_decode($data["cllassified_title"], ENT_QUOTES);
					$discussion_text = $data["cllassified_description"] = htmlspecialchars_decode($data["cllassified_description"], ENT_QUOTES);
					//$feed["timeline_text"] =html_entity_decode ($feed["timeline_text"]);
					$discussion["strlen"] = strlen($data["cllassified_description"]);

					$discussion["share_classified_data"] = "Congratulations!! You have received a Business Inquiry from Zoobiz!!";

					if ($discussion_title != "") {
						if ($discussion["share_classified_data"] != null) {
							$discussion["share_classified_data"] .= "\n\nTitle : " . $discussion_title;
						} else {
							$discussion["share_classified_data"] .= "Title : " . $discussion_title;
						}
					}

					if ($discussion_text != '') {
						if (strlen($data["cllassified_description"]) > $charLimit) {
							$discussion["share_classified_data"] .= "\n\nDescription : " . implode(' ', array_slice(explode(' ', $discussion_text), 0, 200)) . '...';
							//'"'.substr($timeline_text, 0, $charLimit).'..."';
						} else {
							$discussion["share_classified_data"] .= "\n\nDescription : " . substr($discussion_text, 0, $charLimit);
						}
					}

					$discussion["share_classified_data"] .= "\n\nName : " . $user_data_arr['user_full_name'];

					if ($user_data_arr["user_mobile"] != "" && $user_data_arr["mobile_privacy"] == false) {
						$discussion["share_classified_data"] .= "\nMobile No. : " . $user_data_arr["user_mobile"];
					}

					$userDataQuery = $d->selectRow("category_name,sub_category_name,company_name", "users_master,user_employment_details,business_categories,business_sub_categories", "users_master.user_id = '$data[user_id]' AND users_master.user_id = user_employment_details.user_id
						AND user_employment_details.business_category_id = business_categories.business_category_id
					...
					if (mysqli_num_rows($userDataQuery) > 0) {
						...
						while ($data_user = mysqli_fetch_array($userDataQuery)) {
							if ($data_user["sub_category_name"] != "") {
								$discussion["share_classified_data"] .= "\nBusiness Category : " . html_entity_decode($data_user["sub_category_name"]);
							}
							if ($data_user["company_name"] != "") {
								$discussion["share_classified_data"] .= "\nCompany Name : " . html_entity_decode($data_user["company_name"]);
							}
						}
					}
			...
			if (count($CArray) > 0) {
				...
				for ($mc = 0; $mc < count($CArray); $mc++) {
					...
					if (strtotime($data_comment['created_date']) < strtotime('-30 days')) {
						$comment["created_date"] = date("j M Y", strtotime($data_comment['created_date']));
					} else {
